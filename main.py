...
def load_data():
    ...
    try:
        ...
        data = {
            "users": {},
            "prices": load_prices(),
            "authorized_users": load_authorized_users_from_db(),
            "admin_ids": load_admins_from_db()
        }
        ...
        for user in users:
            data["users"][str(user["user_id"])] = {
                ...
                "orders": load_user_orders(str(user["user_id"])),
                "topups": load_user_topups(str(user["user_id"]))
            }
...
def load_data_from_file():
    """Fallback to file-based data loading"""
...
def save_data(data):
    ...
    try:
        ...
        for user_id, user_data in data["users"].items():
            ...
            # Save user orders
            save_user_orders(str(user_id), user_data.get("orders", []))
            
            # Save user topups
            save_user_topups(str(user_id), user_data.get("topups", []))
            
        # Save other data
        save_prices(data.get("prices", {}))
        save_authorized_users_to_db(data.get("authorized_users", []))
        save_admins_to_db(data.get("admin_ids", [ADMIN_ID]))
...
def save_data_to_file(data):
    """Fallback to file-based data saving"""
    with open(DATA_FILE, "w") as f:
    ...
    try:
        ...
        if auth_collection:
            auth_collection.update_one(
                {"type": "authorized_list"},
                {"$set": {"users": users_list, "updated_at": datetime.now()}},
                upsert=True
            )
    except Exception as e:
        print(f"Error saving authorized users to DB: {e}")
    ...
    try:
        ...
        if admins_collection:
            admins_collection.update_one(
                {"type": "admin_list"},
                {"$set": {"admin_ids": admin_list, "updated_at": datetime.now()}},
                upsert=True
            )
    except Exception as e:
        print(f"Error saving admins to DB: {e}")
...
def load_prices():
    ...
    try:
        ...
        if prices_collection:
            doc = prices_collection.find_one({"type": "diamond_prices"})
            return doc.get("prices", {}) if doc else {}
    except:
        pass
    return {}
    ...
    try:
        ...
        if prices_collection:
            prices_collection.update_one(
                {"type": "diamond_prices"},
                {"$set": {"prices": prices_dict, "updated_at": datetime.now()}},
                upsert=True
            )
    except Exception as e:
...
def load_user_orders(user_id):
    ...
    try:
        ...
        if orders_collection:
            orders = orders_collection.find({"user_id": str(user_id)})
            return list(orders)
    except:
        pass
    return []
...
def load_user_topups(user_id):
    ...
    try:
        ...
        if topups_collection:
            topups = topups_collection.find({"user_id": str(user_id)})
            return list(topups)
    except:
        pass
    return []
